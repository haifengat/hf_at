...
namespace HaiFeng
{
	public class DataProcess
	{
		...
		/// <summary>
		/// 过期,放弃.改用UpdateInfo(Mongo....)
		/// </summary>
		public void UpdateInfo()
		{
			foreach (string url in new[] { "tradingtime.json", "tradedate.csv", "commission.json" })
			{
				HttpWebRequest req = (HttpWebRequest)WebRequest.Create("http://106.39.36.80:10001/" + url);
				req.Credentials = new NetworkCredential("hetao", "54hetao");
				...
				File.WriteAllText(req.RequestUri.ToString().Split('/').Last(), lines);
			}
		...
		void ReadTradeDate()
		{
			if (!File.Exists("TradeDate.csv"))
			{
				Console.Write("无交易日历数据错误");
				return;
			}
			_tradeDates.Clear();
			foreach (string line in File.ReadAllLines("TradeDate.csv"))
			{
				string[] fields = line.Split(',');
				if (fields[1].Trim('"') != "1")
					continue;
				_tradeDates.Add(fields[0].Trim('"'));
			}
			_tradeDates.Sort();
		}

		void ReadTradingTime()
		{
			if (File.Exists("TradingTime.json"))
			{
				string lines = File.ReadAllText("TradingTime.json");
				var list = JsonConvert.DeserializeObject<List<WorkingTimeGroup>>(lines);
				foreach (var wt in list)
				{
					_workingTimes[wt._id] = wt;
				}
			}
			else
				Console.WriteLine("无 TradingTime.xml, 无法处理Tick修正.");
		}

		void ReadProductInfo()
		{
			if (File.Exists("commission.json"))
			{
				string lines = File.ReadAllText("commission.json");
				var list = JsonConvert.DeserializeObject<List<ProductInfo>>(lines);
